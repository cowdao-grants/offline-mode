...
function parseArgs(): ParsedArgs {
  ...
  if (!parsed.from?.startsWith("0x") || parsed.from.length !== 66) {
    console.error(
      "Error: Please provide a valid private key (0x... 66 characters)"
    );
    process.exit(1);
  }
...
function getTokenAddress(symbol: string): string {
  ...
  if (!address) {
    ...
    console.error(
      `Available tokens: ${Object.keys(addresses.tokens).join(", ")}`
    );
    process.exit(1);
  }
  return address;
...
async function checkOrderStatus(orderUid: string): Promise<string> {
  try {
    const response = await fetch(
      `http://${CONFIG.host}/api/v1/orders/${orderUid}`
    );
...
async function main() {
  ...
  console.log(
    `  Sell: ${args.sellToken} (${sellTokenAddress}) - ${sellTokenDecimals} decimals`
  );
  console.log(
    `  Buy: ${args.buyToken} (${buyTokenAddress}) - ${buyTokenDecimals} decimals`
  );
  ...
  console.log(
    `  Trader has ${formatBalance(balance, sellTokenDecimals)} ${
      args.sellToken
    }`
  );
  console.log("");
  ...
  const amountField = args.sellAmount
    ? "sellAmountBeforeFee"
    : "buyAmountAfterFee";
  const amountValue = args.sellAmount
    ? parseAmount(args.sellAmount)
    : parseAmount(args.buyAmount!);

  if (args.sellAmount) {
    console.log(
      `Creating order: Sell ${args.sellAmount} ${args.sellToken} for ${args.buyToken}`
    );
  } else {
    console.log(
      `Creating order: Buy ${args.buyAmount} ${args.buyToken} with ${args.sellToken}`
    );
  }
  console.log("");
  ...
  const adjustedBuyAmount = (
    (BigInt(quoteBuyAmount) * BigInt(Math.floor(surplusMultiplier * 10000))) /
    10000n
  ).toString();

  console.log("  Quote received:");
  console.log(
    `    Sell amount: ${formatBalance(
      BigInt(quoteSellAmount),
      sellTokenDecimals
    )} ${args.sellToken}`
  );
  console.log(
    `    Buy amount (from quote): ${formatBalance(
      BigInt(quoteBuyAmount),
      buyTokenDecimals
    )} ${args.buyToken}`
  );
  console.log(
    `    Buy amount (with ${args.surplusPercent}% surplus): ${formatBalance(
      BigInt(adjustedBuyAmount),
      buyTokenDecimals
    )} ${args.buyToken}`
  );
  console.log(
    `    Potential surplus: ${formatBalance(
      BigInt(quoteBuyAmount) - BigInt(adjustedBuyAmount),
      buyTokenDecimals
    )} ${args.buyToken}`
  );
  ...
  console.log(
    `  ${args.sellToken}: ${formatBalance(finalSellBalance, sellTokenDecimals)}`
  );
  console.log(
    `  ${args.buyToken}: ${formatBalance(finalBuyBalance, buyTokenDecimals)}`
  );
  console.log("");
