...
export async function deployAuxiliary(
  ...
  logger.debug('Fetching Balances Helper bytecode from mainnet');
  const balancesBytecode = execSync(
    `cast code ${MAINNET_BALANCES_HELPER} --rpc-url ${mainnetRpcUrl}`,
    { encoding: 'utf8' }
  ).trim();

  logger.trace(indent(`Balances Helper bytecode length: ${balancesBytecode.length} bytes`));

  logger.debug('Setting Balances Helper bytecode at local address');
  execSync(
  ...
  logger.info('Balances Helper contract deployed successfully');
  logger.info('Deployed Balances Helper address:');
  ...
  logger.debug('Fetching Signatures bytecode from mainnet');
  const signaturesBytecode = execSync(
    `cast code ${mainnetSignatures} --rpc-url ${mainnetRpcUrl}`,
    { encoding: 'utf8' }
  ).trim();

  logger.trace(indent(`Signatures bytecode length: ${signaturesBytecode.length} bytes`));

  logger.debug('Setting Signatures bytecode at local address');
  execSync(
  ...
  logger.info('Signatures contract deployed successfully');
  logger.info('Deployed Signatures address:');
  ...
  logger.debug('Setting up HooksTrampoline deployer account');
  execSync(
  ...
  logger.debug('Deploying HooksTrampoline at mainnet address');
  ...
  logger.info('HooksTrampoline contract deployed successfully');
  logger.info('Deployed HooksTrampoline address:');
  ...
  logger.debug('Building CoWShed contracts');
  ...
  logger.info('CoWShed contracts deployed successfully');
  logger.info('Deployed CoWShed addresses:');
